# ppds-labs: dining_philosophers_footman
...
def get_forks(shared, philosopher_id):
    print("Philosopher {:2d} is waiting for forks.".format(philosopher_id))
    sleep(randint(1, 10) / 10)  # sleep or print above helps with starving
    shared.footman.wait()
    print("Philosopher {:2d} is getting forks.".format(philosopher_id))
...
def put_forks(shared, philosopher_id):
    print("Philosopher {:2d} is putting down forks.".format(philosopher_id))
...
def main():
    n = 10  # n philosophers, n forks
    eat_limit = n - 1
    eat_coef = 1.0
    shared = Shared(n, eat_limit, eat_coef)
    threads = []
    for i in range(n):
